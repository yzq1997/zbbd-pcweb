        ...
        <a-modal :maskClosable="false" v-model="addVisible" @ok="addOk" okText="确定" cancelText="取消">
            ...
            <a-form :form="addForm">
                <a-form-item label="登录名" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
                    <a-input placeholder="请输入，同时包括英文及数字，区分大小写" v-decorator="['LoginName', { rules: [{ required: true, message: '请输入登录名' },{max:12,message:'登录名字数长度在12以内'},{min:6,message:'登录名字数长度在6以上'},{pattern:/^(?=.*[0-9])(?=.*[a-zA-Z])(.{0,})$/,message:'格式有误,同时包括英文及数字,区分大小写'}] }]"/>
                </a-form-item>
                <a-form-item label="用户名" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
                    <a-input placeholder="请输入真实姓名" v-decorator="['UserName', { rules: [{ required: true, message: '请输入用户名' },{ max: 10, message: '用户名字数长度在10以内' }] }]"/>
                </a-form-item>
				<a-form-item label="密码" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
				    <a-input placeholder="请输入密码" v-decorator="['PassWord', { rules: [{ required: true, message: '请输入密码' },{ min: 10, message: '密码长度不小于6' }] }]"/>
				</a-form-item>
        ...
        <a-modal :maskClosable="false" v-model="editVisible" @ok="editOk" okText="确定" cancelText="取消">
            ...
            <a-form :form="editForm">
                ...
                <a-form-item label="登录名" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                    <a-input placeholder="请输入，同时包括英文及数字，区分大小写" disabled v-decorator="['LoginName', { rules: [{ required: true, message: '请输入登录名' },{ min: 6, message: '登录名字数长度在6以上' },{ max: 12, message: '登录名字数长度在12以内' }] }]"/>
                </a-form-item>
                <a-form-item label="用户名" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                    <a-input placeholder="请输入真实姓名" v-decorator="['UserName',{ rules: [{ required: true, message: '请输入用户名' },{ max: 10, message: '用户名字数长度在10以内' }] }]"/>
                </a-form-item>
...
export default {
    ...
    methods:{
        ...
        confirm(record){
            let that = this;
            this.$dataApi({
                url:'/api/Sys_Users/ResetPassword',
                method:'POST',
                data:{
                    Id:record.record.Id
                }
		...
		exportExcel(){
			...
			require.ensure([], () => {
			    ...
			    const tHeader = ['登录名', '用户名','创建时间'];   // 上面设置Excel的表格第一行的标题
			    const filterVal = ['LoginName', 'UserName', 'JoinDate']; // 上面的index、nickName、name是tableData里对象的属性key值
