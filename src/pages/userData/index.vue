		...
		exportExcel(){
			...
			this.$dataApi({
				...
				require.ensure([], () => {
					...
					const tHeader = ['序号','微信昵称', '注册时间','是否会员','邀请码'];   // 上面设置Excel的表格第一行的标题
					const filterVal = ['num','nickname', 'RegistTime','IsMember','UniqueInvitationCode']; // 上面的index、nickName、name是tableData里对象的属性key值
