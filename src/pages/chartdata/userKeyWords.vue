	...
	export default{
		...
		methods:{
			...
			getUserAccesslist(e){  //获取数据
				...
				this.$dataApi({
                    ...
                    console.log(res);
					this.cleardata();
					res.KeywordAnalysisList.sort(this.datesort)
					that.paginations.total  = res.TotalCount;
					that.tableData = res.KeywordAnalysisList;
					res.KeywordAnalysisList.forEach((item,index)=>{
						...
						that.ActiveSetCount.push(item.ActiveSetCount);
						that.PassiveSetCount.push(item.PassiveSetCount);
						that.NotSetCount.push(item.NotSetCount);
					})
			...
			exportExcel(){
				...
				this.$dataApi({
					...
					res.KeywordAnalysisList.sort(this.datesort)
					that.exportdata = res.KeywordAnalysisList;
					...
					require.ensure([], () => {
						...
						const tHeader = ['序号','时间', '主动设置','被动设置','未设置'];   // 上面设置Excel的表格第一行的标题
						const filterVal = ['num','JoinDate', 'ActiveSetCount','PassiveSetCount', 'NotSetCount']; // 上面的index、nickName、name是tableData里对象的属性key值
						const list = this.exportdata;  //把要导出的数据tableData存到list
						const data = this.formatJson(filterVal, list);
						export_json_to_excel(tHeader, data, '用户关键词统计统计');//最后一个是表名字
					})
				})
			},
		...
		created(){
			this.columns = [
			    ...
			    {
			        title: '主动设置',
			        dataIndex: 'ActiveSetCount',
					 
			    },
			    {
			        title: '被动设置',
			        dataIndex: 'PassiveSetCount',
