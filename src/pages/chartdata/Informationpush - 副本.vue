    ...
    exportExcel() {
      ...
      this.$dataApi({
        ...
        require.ensure([], () => {
          ...
          const tHeader = ["序号", "时间", "推送用户总量", "推送免费用户量","推送会员用户量","总点击量", "免费用户点击量","会员用户点击量"]; // 上面设置Excel的表格第一行的标题
          const filterVal = ["num", "Time","TotalPushCount", "FreePushCount", "VIPPushCount","TotalClickCount","FreeClickCount","VIPClickCount"]; // 上面的index、nickName、name是tableData里对象的属性key值
