	...
	<div class="index">
		...
		<div class="table" >
			...
			<a-col :span="24" style="height: auto;">
			    <tables  :columns="columns" :scroll="widthFlag" :tableData="tableData" size="middle"  :loading="loading" :handleTableChange="handleTableChange" :bordered="bordered" :pagination="paginations" >
			    </tables>
			</a-col>
		</div>
	</div>
	...
	export default{
		...
		data(){
			return {
				...
				FreeUV:[], //(免费用户独立访客数)
				VIPUV:[], //(会员用户独立访客数)
				SetmealAUV:[], //(套餐A用户独立访客数)
				SetmealBUV:[], //(套餐B用户独立访客数)
				SetmealCUV:[], //(套餐C用户独立访客数)
				PV:[], //(总访问量)
				UV:[], //(总访客数)
			}
		},
			...
			getUserAccesslist(e){  //获取数据
				...
				this.$dataApi({
					...
					res.Rows.forEach((item,index)=>{
						...
						that.SetmealAPV.push(item.SetmealAPV)
						that.SetmealBPV.push(item.SetmealBPV)
						that.SetmealCPV.push(item.SetmealCPV)
						that.FreeUV.push(item.FreeUV)
						that.VIPUV.push(item.VIPUV)
						that.SetmealAUV.push(item.SetmealAUV)
						that.SetmealBUV.push(item.SetmealBUV)
						that.SetmealCUV.push(item.SetmealCUV)
						that.PV.push(item.FreePV+item.VIPPV+item.SetmealAPV+item.SetmealBPV+item.SetmealCPV)
						that.UV.push(item.FreeUV+item.VIPUV+item.SetmealAUV+item.SetmealBUV+item.SetmealCUV)
					})
					that.tableData.sort(this.tabledatesort)
					that.tableData.forEach((item,index)=>{
						item.num=index+1;
						item.PV=item.FreePV+item.VIPPV+item.SetmealAPV+item.SetmealBPV+item.SetmealCPV;
						item.UV=item.FreeUV+item.VIPUV+item.SetmealAUV+item.SetmealBUV+item.SetmealCUV
					})
					this.getchartinfo()
				})
			},
			...
			exportExcel(){
				...
				this.$dataApi({
					...
					require.ensure([], () => {
						...
						const tHeader = ['序号','时间', '总访问量','免费用户访问量','会员用户访问量','套餐A用户访问量','套餐B用户访问量','套餐C用户访问量','总访客数','免费用户独立访客数','会员用户独立访客数','套餐A用户独立访问量','套餐B用户独立访问量','套餐C用户独立访问量'];   // 上面设置Excel的表格第一行的标题
						const filterVal = ['num','Time', 'PV','FreePV', 'VIPPV','SetmealAPV','SetmealBPV','SetmealCPV','UV', 'FreeUV','VIPUV','SetmealAUV','SetmealBUV','SetmealCUV']; // 上面的index、nickName、name是tableData里对象的属性key值
		...
		created(){
			this.columns = [
                ...
                {
			        title: '套餐A用户访问量',
			        dataIndex: 'SetmealAPV',
					 
                },
                {
			        title: '套餐B用户访问量',
			        dataIndex: 'SetmealBPV',
					 
                },
                {
			        title: '套餐C用户访问量',
			        dataIndex: 'SetmealCPV',
                ...
                {
			        title: '套餐A用户独立访客数',
			        dataIndex: 'SetmealAUV',
					 
                },
                {
			        title: '套餐B用户独立访客数',
			        dataIndex: 'SetmealBUV',
					 
                },
                {
			        title: '套餐C用户独立访客数',
			        dataIndex: 'SetmealCUV',
