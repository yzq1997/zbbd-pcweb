<template>
	<div class="index">
		<div class="chart">
            ...
            <div class="dataNum">
                <p class="dataNumP1">累计用户量</p>
                <p class="dataNumP2">{{userNum}}</p>
            </div>
		</div>
		<div class="table" >
	...
	export default{
		...
		methods:{
			...
			exportExcel(){
				...
				let that = this;
				this.$dataApi({
				    url:'api/UserDataStatistics/UserAreaStatistics',
				    method:'POST',
				    data:{
                    ...
                    console.log(res);
                    // that.exportdata = res;
                    res.UserAreaStatisticsList.sort(this.datesort)
					that.exportdata = res.UserAreaStatisticsList;
					...
					if(that.exportdata.length > 0){
						require.ensure([], () => {
						    ...
						    const tHeader = ['序号','地区', '用户量'];   // 上面设置Excel的表格第一行的标题
						    const filterVal = ['num','Province', 'Count']; // 上面的index、nickName、name是tableData里对象的属性key值
						    const list = this.exportdata;  //把要导出的数据tableData存到list
						    const data = this.formatJson(filterVal, list);
						    export_json_to_excel(tHeader, data, '用户地区统计');//最后一个是表名字
						  }) 
					}
				})
			},
			...
			GetBaseDataStatByTime(){
				...
				this.$dataApi({
                    ...
                    that.userNum = res.TotalUserCount
					that.paginations.total  = res.TotalCount;
                    that.tableData = res.UserAreaStatisticsList;
