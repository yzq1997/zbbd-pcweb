	...
	export default{
			...
			exportExcel(){
				...
				this.$dataApi({
					...
					require.ensure([], () => {
						...
						const tHeader = ['序号','时间', '免费用户','邀请码会员','注册会员'];   // 上面设置Excel的表格第一行的标题
						const filterVal = ['num','JoinDate', 'FreeUsersCount','InvitationCodeMemberCount','RegisteredMembersCount']; // 上面的index、nickName、name是tableData里对象的属性key值
		...
		created(){
			this.columns = [
			    ...
			    {
			        title: '邀请码会员',
			        dataIndex: 'InvitationCodeMemberCount',
