<template>
	<div class="index">
		<div class="chart">
			<div class="searchinfo">
				<a-row style="overflow:hidden;margin-bottom:20px;height:40px;" type="flex" justify="space-between">
                    ...
                    <a-col :span="2">
						<a-select default-value="全部" style="width: 150px" @change="handleChange1">
                            <a-select-option value="">全部</a-select-option>
                            <a-select-option value="搜索">搜索</a-select-option>
                            <a-select-option value="招标公告">招标公告</a-select-option>
                            <a-select-option value="变更公告">变更公告</a-select-option>
                            <a-select-option value="结果公告">结果公告</a-select-option>
                            <a-select-option value="数据导出">数据导出</a-select-option>
                            <a-select-option value="公告类型">公告类型</a-select-option>
                            <a-select-option value="时间">时间</a-select-option>
                            <a-select-option value="地区">地区</a-select-option>
                            <a-select-option value="我关注的项目">我关注的项目</a-select-option>
                            <a-select-option value="招标推送设置">招标推送设置</a-select-option>
                            <a-select-option value="我的订阅维护">我的订阅维护</a-select-option>
                            <a-select-option value="免费注册">免费注册</a-select-option>
                            <a-select-option value="常见问题查询">常见问题查询</a-select-option>
                            <a-select-option value="抽奖">抽奖</a-select-option>
						</a-select>
					</a-col>
					<a-col :span="2">
		...
		<div class="table" >
            <div class="tabs">
                <span class="tabsHome" @click="tabsBox(1)" :class="tabsT?'tabsTrue':'tabsFalse'">首页</span>
                <span class="tabsMy" @click="tabsBox(2)" :class="!tabsT?'tabsTrue':'tabsFalse'">我的</span>
            </div>
			<a-col :span="24" style="margin-bottom:20px;height: auto;">
                <a-col :span="9">
					<a-form-item label="时间:" style="display:flex">
	...
	export default{
		...
		data(){
			return {
                ...
                tabsT:true,
                EventName:'',  //事件名称
                JudgeValue:0,  //0首页  1我的
                TrggerTimes:[],//触发次数
                TrggerUsers:[],//触发人数
                BeginRegisterTime:'',  //点击查询的开始时间
                EndRegisterTime:'',  //点击查询的结束时间
                flagTime1:'',
                flagTime2:'',
			}
		},
		methods:{
            ...
            onChange1(date, dateString) {
                ...
                if(dateString[0] == ""){
                    this.BeginRegisterTime = this.flagTime1;
                    this.EndRegisterTime =  this.flagTime2;
                }
            ...
            getUserAccesslist(e){  
				...
				this.$dataApi({
                    ...
                    console.log(res);
					this.cleardata();
					res.UserMenuAnalysisList.sort(this.datesort)
					// that.paginations.total  = res.TotalCount;
					// that.tableData = res.UserMenuAnalysisList;
					res.UserMenuAnalysisList.forEach((item,index)=>{
						...
						that.TrggerTimes.push(item.TrggerTimes)
						that.TrggerUsers.push(item.TrggerUsers)
					})
			...
			getUserAccesslist1(e){  
				...
				let that = this;
				this.$dataApi({
					url:'api/UserDataStatistics/UserMenuAnalysisEvent',
					method:'POST',
					data:{
						BeginTime:that.BeginRegisterTime,
						EndTime: that.EndRegisterTime,
                        IsImport: 1,
                        PageNo: "1",//当前页面
						PageSize: "10",//每页数量
						// PageNo:that.params.pageNo?that.params.pageNo:1,//当前页面
                        // PageSize: 10,//每页数量
                        // EventName:this.EventName,
			...
			exportExcel(){
				...
				this.$dataApi({
					...
					res.UserMenuAnalysisList.sort(this.datesort)
					that.exportdata = res.UserMenuAnalysisList;
					...
					require.ensure([], () => {
						...
						const tHeader = ['序号','事件', '时间','触发次数','触发人数','人均触发次数'];   // 上面设置Excel的表格第一行的标题
						const filterVal = ['num','EventName', 'JoinDate','TrggerTimes', 'TrggerUsers','AvgTrggerUsers']; // 上面的index、nickName、name是tableData里对象的属性key值
						const list = this.exportdata;  //把要导出的数据tableData存到list
						const data = this.formatJson(filterVal, list);
						export_json_to_excel(tHeader, data, '菜单分析');//最后一个是表名字
					})
				})
			},
			...
			initPrams(){
                ...
                this.flagTime1 = this.params.beginDate;
                this.flagTime2 = this.params.endDate;
                this.BeginRegisterTime = this.params.beginDate;
                this.EndRegisterTime = this.params.endDate;
			}
		...
		created(){
			this.columns = [
			    ...
			    {
			        title: '触发次数',
			        dataIndex: 'TrggerTimes',
					 
			    },
			    {
			        title: '触发人数',
			        dataIndex: 'TrggerUsers',
					 
				},
				{
			        title: '人均触发次数',
			        dataIndex: 'AvgTrggerUsers',
