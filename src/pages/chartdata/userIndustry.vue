<template>
	<div class="index">
		<div class="chart">
			<div class="searchinfo">
				<a-row style="overflow:hidden;margin-bottom:20px;height:40px;" type="flex" justify="space-between">
				    <a-col :span="4" class="bold visiT">
						<span>用户行业统计</span>
					</a-col>
		
					<a-col :span="2">
						<a-select default-value="最近7天" style="width: 120px" @change="handleChange">
						      <a-select-option value="7">
						        最近7天
						      </a-select-option>
						      <a-select-option value="15">
						        最近15天
						      </a-select-option>
						      <a-select-option value="30">
						        最近30天
						      </a-select-option>
						</a-select>
					</a-col>
				    <a-col :span="8">
				        <a-form :form="filterForm">
				            <a-form-item label="" :colon="false" :label-col="{ span: 4 }" :wrapper-col="{ span: 22 }">
				                <a-locale-provider :locale="zhCN">
				                    <a-range-picker :disabled-date="disabledDate" style="width:100%;" @change="onChange" :allowClear="false" :placeholder="['开始时间', '结束时间']" v-decorator="['dateRange',initValue]" />
				                </a-locale-provider>
				            </a-form-item>
				        </a-form>
				    </a-col>
				</a-row>
			</div>
			<div class="chartinfo" ref="chartinfo"></div>
		</div>
		<div class="table" >
            <div>
                <a-col :span="24" style="margin-bottom:20px;height: auto;">
                    <a-row>
                        <a-col :span="4">
                            <a-button type="primary" @click="exportExcel">导出Execl</a-button>
                        </a-col>
                    </a-row>
                </a-col>
                <a-col :span="24" style="height: auto;">
                    <tables  :columns="columns" :tableData="tableData" size="middle"  :loading="loading" :handleTableChange="handleTableChange" :bordered="bordered" :pagination="paginations" :option="option1">
                    </tables>
                </a-col>
            </div>
            <div>
                <a-col :span="24" style="height:52px;">
                    <a-row>
                        <a-col :span="8">
                            <a-form-item style="display:flex">
                                <a-range-picker @change="onChange1" :disabled-date="disabledDate" />
                            </a-form-item>
                        </a-col>
                        <a-col :span="8" style="margin-top:4px;margin-left:5px">
                            <a-button type="primary" @click="filterSearch" html-type="submit">查询</a-button>
                            <a-button type="primary" @click="newAdd">新增</a-button>
                        </a-col>
                    </a-row>
                </a-col>
                <a-col :span="24" style="height: auto;">
                    <tables :columns="columns1" :tableData="tableData1" size="middle"  :loading="loading" :handleTableChange="handleTableChange1" :bordered="bordered" :pagination="paginations1" :option="option">
                    </tables>
                </a-col>
            </div>
			
		</div>

        <a-modal v-model="visible" title="添加关键词" @ok="handleOk" @cancel="handleNo">
            <a-input placeholder="请输入关键词" v-model="inputVal" />
        </a-modal>

        <a-modal v-model="visible1" title="删除行业" @ok="handleOk1" >
            <p>确定删除改行业么？</p>
        </a-modal>
	</div>
</template>

<script>
	import tables from '@/pages/common/table'
	import zhCN from 'ant-design-vue/lib/locale-provider/zh_CN';
	import moment from 'moment';
	import { formatDate } from '../../utils/dateformat.js'
	import Bus from '../../utils/vue-bus.js';
	export default{
		components:{
		    tables
		},
		data(){
			return {
				zhCN,
				xdata:[],
				ydata: [],
				columns:[],
				columns1:[],
				tableData:[],
				tableData1:[],
				loading:false,
                bordered:true,
                paginations: {
				    current:1,
				    pageSize:20,
				    total:0,
				    size:'default',
				    showQuickJumper:true,
				    showTotal:function(total,range){
				        return  `共 ${total} 条记录`;
				    }
				},
				paginations1: {
				    current:1,
				    pageSize:10,
				    total:0,
				    size:'default',
				    showQuickJumper:true,
				    showTotal:function(total,range){
				        return  `共 ${total} 条记录`;
				    }
                },
				endDay:"",
				startDay:"",
				filterForm:this.$form.createForm(this,{name:'filter'}),
				params:{},
				params1:{},
				daynum:7,
				exportdata:[],
				filterForm:this.$form.createForm(this),
				source:0,
				Time:[], //(日期)
				FreePV:[], //(免费用户访问量)
				VIPPV:[], //( 会员用户访问量)
				FreeUV:[], //(免费用户独立访客数)
				VIPUV:[], //(会员用户独立访客数)
				PV:[], //(总访问量)
                UV:[], //(总访客数)
                UserCount:[], //用户量
                KeyWordCount:[],  //关键词量
                option:[
                    {
                        type:"detail",
                        name:"删除",
                        fn:(e)=>{
                            this.detail(e);
                        }
                    }
                ],
                option1:[
                    {
                        type:"detail",
                        name:"查看",
                        fn:(e)=>{
                            this.tableClick(e);
                        }
                    }
                ],
                visible:false,
                visible1:false,
                inputVal:'',
                ID:1,//行业ID
                KeyWordDel:'', //删除的关键词
                time1:'',
                time2:'',
			}
		},
		methods:{
            moment,
            disabledDate(current) {
                // Can not select days before today and today
                return current && current > moment().endOf('day');
            },
			// 表格事件
			handleTableChange(pagination,filter,sort){
				let that = this;
				const pager = Object.assign(this.paginations);
				pager.current = pagination.current;
				this.paginations = pager;
				this.params['pageNo'] = this.paginations['current'];
				this.getUserAccesslist('false');
            },

            // 表格事件2
			handleTableChange1(pagination,filter,sort){
                // console.log(pagination.current);
				let that = this;
				const pager = Object.assign(this.paginations1);
				pager.current = pagination.current;
				this.paginations1 = pager;
				this.params1['pageNo'] = this.paginations1['current'];
				this.getUserAccesslist1('false');
            },
            
            //查看
            tableClick(e){
                this.paginations1.current = 1;
                this.params1.pageNo = 1;
                // console.log(this.paginations1);
                this.ID = e.ID;
                this.getUserAccesslist1();
            },
            
            //点击查询
            filterSearch(){
                this.getUserAccesslist1();
            },

            onChange(date, dateString) {
				// console.log(dateString);
				
				this.params.beginDate=dateString[0];
                this.params.endDate=dateString[1];
                // console.log(this.params);
				this.getUserAccesslist(0);
			},

            //时间
            onChange1(date, dateString) {
                // console.log(dateString);
                
                this.time1 = dateString[0];
                this.time2 = dateString[1];
                // console.log(dateString[0]);
                if(dateString[0] == ""){
                    this.time1 = '';
                    this.time2 = '';
                }
                // console.log(this.time1);
                // this.params.beginDate=dateString[0];
                // this.params.endDate=dateString[1];
                
            },

            //新增
            newAdd(){
                this.visible = true;
            },

            //确定增加
            handleOk(e) {
                // console.log(this.inputVal);
                
                let that = this;
                this.visible = false;
                this.$dataApi({
					url:'api/UserDataStatistics/AddIndustryKeyWord',
					method:'POST',
					data:{
                        ID:that.ID,
                        KeyWord:that.inputVal
					}
				}).then(res=>{
                    if(res.Status == 409){
                        that.$message.info('已有关键词');
                        return false;
                    }else{
                        that.inputVal = '';
                        that.getUserAccesslist1()
                    }
                    
                })
            },

            //删除行业
            handleOk1(){
                this.visible1 = false;
                let that = this;
                this.visible = false;
                this.$dataApi({
					url:'api/UserDataStatistics/DeleteIndustryKeyWord',
					method:'POST',
					data:{
                        ID:that.ID,
                        KeyWord:this.KeyWordDel
					}
				}).then(res=>{
                    console.log(res);
                    // that.inputVal = '';
                    that.getUserAccesslist1()
                })
            },

            //取消新增
            handleNo(){
                // console.log(this.inputVal);
                this.inputVal = '';
            },

            //删除
            detail(e){
                this.visible1 = true;
                // console.log(e);
                this.KeyWordDel = e.KeyWord
            },

			getchartinfo(){
				let myChart = this.$echarts.init(this.$refs.chartinfo);
				myChart.setOption({
					tooltip: {              //设置tip提示
						trigger: 'axis'
                    },
					legend: {
						data: ['用户量', '关键词']
					},
					xAxis: {
						name:"行业",
						type: 'category',
						// data:this.xdata,
						data:['农林牧渔','行政设施','交通工程','信息技术','弱电安防','能源化工','水利水电','机械设备','服务采购','行政办公','医疗卫生','建筑工程',],
					},
					yAxis: {
						name:"数量",
						type: 'value'
					},
					series: [
						{
							data: this.UserCount,
							type: "bar",
							name: '用户量',
						},
						{
							name: '关键词',
							type: 'bar',
							data: this.KeyWordCount,
							color:"#006BDF",
						},
					]
				})
			},
	
			handleChange(value){
				// console.log(value);
				this.daynum=value;
				let date1 = new Date();
				let date2 = new Date(date1);
				let emouth=date1.getMonth() + 1;
				let eday=date1.getDate();
				if(emouth < 10){
					emouth="0"+emouth;
				}
				if(eday <10){
					eday="0"+eday;
				}
				this.endDay=date1.getFullYear() + "-" + emouth + "-" + eday;
				date2.setDate(date1.getDate() - value);
				let smouth=date2.getMonth() + 1;
				let sday=date2.getDate();
				if(smouth < 10){
					smouth="0"+smouth;
				}
				if(sday <10){
					sday="0"+sday;
				}
				this.startDay=date2.getFullYear() + "-" + smouth + "-" + sday;
				
				this.params.beginDate=this.startDay;
				this.params.endDate=this.endDay;
				let source = 0
				this.filterForm.validateFields((error, value) => {
					if (!error) {
						// console.log(value);
						source = value
						// replyState = value.nature;
						// source = Number(value.fenlei)
						
					}
				});
				// console.log(source);
				
				this.getUserAccesslist();
            },
            
			
			datesort:function(x,y){
				return x.time > y.time ? 1 : -1;
			},
			tabledatesort:function(x,y){
				return x.time < y.time ? 1 : -1;
			},
			
			cleardata:function(){
				this.xdata=[];
				this.FreePV=[];
				this.VIPPV=[];
				this.FreeUV=[];
				this.VIPUV=[];
				this.PV=[];
                this.UV=[];
                this.UserCount=[];
                this.KeyWordCount=[];
                
			},

            //获取数据-用户行业
			getUserAccesslist(e){  
				this.source = 1
				this.xdata=[];
				this.ydata=[];
				let beginDate=new Date(Date.parse(this.params.beginDate.replace(/-/g,   "/")))
				let endDate=new Date(Date.parse(this.params.endDate.replace(/-/g,   "/")))	
				let that = this;
				this.$dataApi({
					url:'api/UserDataStatistics/UserIndustryStatistics',
					method:'POST',
					data:{
						BeginTime:formatDate(beginDate, 'yyyy-MM-dd'),
						EndTime: formatDate(endDate, 'yyyy-MM-dd'),
						IsImport: 1,
						// PageNo: 1,//当前页面
                        // PageSize: 20,//每页数量
                        PageNo: "1",//当前页面
						PageSize: "10",//每页数量
					}
				}).then(res=>{
                    console.log(res);
					this.cleardata();
					
					res.UserIndustryStatisticsList.sort(this.datesort)
					that.paginations.total  = res.TotalCount;
					that.tableData = res.UserIndustryStatisticsList;
					res.UserIndustryStatisticsList.forEach((item,index)=>{
						that.xdata.push(item.Time)
						that.UserCount.push(item.UserCount)
						that.KeyWordCount.push(item.KeyWordCount)
						
					})
					that.tableData.sort(this.tabledatesort)
					that.tableData.forEach((item,index)=>{
						item.num=index+1;
						
					})
					this.getchartinfo()
				})
            },
            
            //获取数据-单个行业
            getUserAccesslist1(e){ 
				// this.source = 1
				this.xdata=[];
				this.ydata=[];
				let beginDate=new Date(Date.parse(this.params.beginDate.replace(/-/g,   "/")))
				let endDate=new Date(Date.parse(this.params.endDate.replace(/-/g,   "/")))	
				let that = this;
				this.$dataApi({
					url:'api/UserDataStatistics/SearchKeyWordByIndustry',
					method:'POST',
					data:{
                        ID:that.ID,
						BeginTime:that.time1,
						EndTime: that.time2,
						IsImport: 0,
						PageNo: this.params1.pageNo,//当前页面
						PageSize: 10,//每页数量
					}
				}).then(res=>{
                    console.log(res);
					// this.cleardata();
					res.IndustryKeyWordList.sort(this.datesort);
					that.paginations1.total  = res.TotalCount;
					that.tableData1 = res.IndustryKeyWordList;
					that.tableData1.sort(this.tabledatesort)
					that.tableData1.forEach((item,index)=>{
						item.num=index+1;
                    });
				})
			},
			
			exportExcel(){
				let beginDate=new Date(Date.parse(this.params.beginDate.replace(/-/g,   "/")))
				let endDate=new Date(Date.parse(this.params.endDate.replace(/-/g,   "/")))	
				let that = this;
				this.$dataApi({
					url:'api/UserDataStatistics/UserIndustryStatistics',
					method:'POST',
					data:{
						BeginTime:formatDate(beginDate, 'yyyy-MM-dd'),
						EndTime: formatDate(endDate, 'yyyy-MM-dd'),
						IsImport: 1,
						pageNo: "1",//当前页面
						pageSize: "10",//每页数量
					}
				}).then(res=>{
					console.log(res);
					
					res.UserIndustryStatisticsList.sort(this.datesort)
					that.exportdata = res.UserIndustryStatisticsList;
					that.exportdata.sort(this.tabledatesort)
					that.exportdata.forEach((item,index)=>{
						item.num=index+1;
						// item.PV=item.FreePV+item.VIPPV;
						// item.UV=item.FreeUV+item.VIPUV
					})
					console.log(that.exportdata)
					require.ensure([], () => {
						const { export_json_to_excel } = require('../../utils/Export2Excel.js');//注意这个Export2Excel路径
						const tHeader = ['序号','行业', '用户量','关键词'];   // 上面设置Excel的表格第一行的标题
						const filterVal = ['num','Industry', 'UserCount','KeyWordCount']; // 上面的index、nickName、name是tableData里对象的属性key值
						const list = this.exportdata;  //把要导出的数据tableData存到list
						const data = this.formatJson(filterVal, list);
						export_json_to_excel(tHeader, data, '用户行业统计');//最后一个是表名字
					})
				})
			},
			
			formatJson(filterVal, jsonData) {
				return jsonData.map(v => filterVal.map(j => v[j]))
            },
            
			//初始化时间
			initPrams(){
				Object.assign(this.params,this.filterForm.getFieldsValue())
				if(Array.isArray(this.params['dateRange']) && this.params['dateRange'].length > 0)
				{
					this.params['beginDate'] = moment(this.params['dateRange'][0]).format('YYYY-MM-DD');
					this.params['endDate'] = moment(this.params['dateRange'][1]).format('YYYY-MM-DD');
					delete this.params['dateRange']
				}
				else
				{
					this.params['beginDate'] = new Date(new Date().getTime() - 24*7*60*60*1000).toLocaleDateString().replace('/','-').replace('/','-');
					this.params['endDate'] = new Date().toLocaleDateString().replace('/','-').replace('/','-');
				}	
			}
			
		},
		created(){
            this.columns = [
			    {
			        title: '序号',
			        dataIndex: 'num',
					 
			    },
			    {
			        title: '行业',
			        dataIndex: 'Industry',
					 
			    },
			    {
			        title: '用户量',
			        dataIndex: 'UserCount',
					 
			    },
			    {
			        title: '关键词',
			        dataIndex: 'KeyWordCount',
					 
                },
                {
			        title: '操作',
			        dataIndex: 'option',
					scopedSlots: { customRender: 'option' },
                },
			];
            this.columns1 = [
			    {
			        title: '关键词',
			        dataIndex: 'KeyWord',
					 
			    },
			    {
			        title: '添加时间',
			        dataIndex: 'JoinDate',
					 
			    },
			    {
			        title: '添加人数',
			        dataIndex: 'AddedPeopleCount',
					 
			    },
			    {
			        title: '操作',
			        dataIndex: 'option',
					scopedSlots: { customRender: 'option' },
                },
            ];
			
			this.initPrams();  //初始化时间
			this.getUserAccesslist(0);
			this.getUserAccesslist1(0);
		},
		mounted(){
			this.getchartinfo()
		},
		computed:{
		    initValue(){
		        let obj = {};
		        obj.initialValue = [moment(new Date(new Date().getTime() - 24*this.daynum*60*60*1000).toLocaleDateString(), 'YYYY/MM/DD'),moment(new Date().toLocaleDateString(), 'YYYY/MM/DD')];
		        return obj;
		    }
		},
		updated() {
			Bus.$emit("collapse",{Height:document.getElementsByClassName('content')[0].clientHeight});
		}
	}
</script>

<style lang="scss" scoped>
	.index{
		width: 100%;
		height: 100%;
		margin: 0 auto;
		.chart{
			width: 100%;
			margin-top: 10px;
			height: auto;
			.searchinfo{
				height: 40px;
				background-color: rgb(255,255,255);
			}
			.searchinfo span{
					font-family: "Microsoft Tai Le Bold", "Microsoft Tai Le Regular", "Microsoft Tai Le";
					font-weight: 700;
					font-style: normal;
					font-size: 16px;
					text-align: center;
					line-height: 40px;
					margin-left: 20px;
				}
			.searchinfo .ant-calendar-picker{
				float: right;
				line-height: 40px;
				margin-right: 10px;
			}
			.searchinfo .ant-select{
				float: right;
				margin-top: 5px;
			}
			.chartinfo{
				height: 400px;
				background-color: rgb(255,255,255);
			}
		}
		.table{
			// width:calc(70% - 30px);
			height:100%;
			margin-top: 20px;
			border-radius:4px;
			padding:20px;
			background-color: rgb(255,255,255);
			overflow: hidden;
            display: flex;
		}
        .table>div{
            width: 48%;
            margin-left: 1%;
        }
	}
</style>
